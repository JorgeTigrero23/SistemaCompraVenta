                    ...
                    <div class="card-body">
                        <div class="form-group row">
                            <div class="col-md-6">
                                <div class="input-group">
                                    ...
                                    <input type="text" v-model="search" @keyup.enter="gerPurchases(1,search,criteria)" class="form-control" placeholder="Texto a buscar">
                                    <button type="submit"  @click="gerPurchases(1,search,criteria)" class="btn btn-primary"><i class="fa fa-search"></i> Buscar</button>
                                </div>
                            </div>
                        </div>
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped table-sm">
                                ...
                                <tbody>
                                    <tr v-for="purchase in arrayPurchases" :key="purchase.id">
                                        <td>
                                            <button type="button" @click="showPurchase(purchase.id)" class="btn btn-success btn-sm">
                                                <i class="icon-eye"></i>
                                            </button>&nbsp;
                                            <template v-if="purchase.status == 'Registrado'">
                                                <button type="button" class="btn btn-danger btn-sm" @click="inactive(purchase.id)">
                                                    <i class="icon-trash"></i>
                                                </button>
                                            </template>
                                        </td>
                                        <td v-text="purchase.username"></td>
                                        <td v-text="purchase.name"></td>
                                        <td v-text="purchase.voucher_type"></td>
                                        <td v-text="purchase.voucher_serie"></td>
                                        <td v-text="purchase.voucher_number"></td>
                    ...
                    <div class="card-body">
                                ...
                                <table class="table table-bordered table-striped table-sm">
                                    ...
                                    <tbody v-if="arrayDetails.length">
                                        <tr v-for="(detail, index) in arrayDetails" :key="detail.id">
                                            ...
                                            <td>
                                                <input v-model="detail.quantity" type="number" value="4" class="form-control">
                                            </td>
                                            <td>
                        ...
                        <div class="form-group row">
                            <div class="col-md-12">
                                ...
                                <button type="button" class="btn btn-primary" @click="save()">Registrar Compra</button>
                            </div>
                        </div>
                    </div>
                </template>
                        ...
                        <div class="form-group row border">
                            <div class="table-responsive col-md-12">
                                <table class="table table-bordered table-striped table-sm">
                                    ...
                                    <tbody v-else>
                                        <tr>
                                        <td colspan="4">No hay articulos agragados</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
...
<script>
    ...
    export default {
        ...
        methods: {    
            gerPurchases(page, search, criteria){
                let me = this;
                let url = '/compra?page=' + page + '&search='+ search + '&criteria=' + criteria;
                axios.get(url).then(function (response) {
                    let result = response.data;
                    me.arrayPurchases = result.purchases.data;
            ...
            selectProvider(search, loading){
                 ...
                 let url = '/proveedor/selectProveedor?filter=' + search;
                axios.get(url).then(function (response) {
                    let result = response.data;
                    // /q: search
                    me.arrayProvider = result.providers;
                    loading(false)
                })
                .catch( function (error) {
                    console.log(error);
            ...
            getDataProvider(value){
                let me = this;

                me.loading = true;
                me.provider_id = value.id;
            },
            searchProduct(){
                let me = this;
                let url = 'articulo/buscar?filter=' + me.barcode;
            ...
            listProducts(search, criteria){
                 let me = this;
                 let url = '/articulo/listaArticulo?search='+ search + '&criteria=' + criteria;
                axios.get(url).then(function (response) {
                    let result = response.data;
            ...
            changePage(page, search, criteria) {
                ...
                // Call methos getPurchases
                me.gerPurchases(page, search, criteria);
            ...
            save(){
                ...
                axios.post('/compra', {
                    ...
                    me.showTemplate = 1;
                    me.gerPurchases(1,'','voucher_number');
                    me.hiddenDetail();
                    me.provider_id = 0;
                    me.voucher_type = '';
                    me.voucher_serie = '';
            ...
            validate(){
                ...
                if(this.provider_id==0) this.errorMessage.push("El Campo Proveedor es obligatorio.  Por favor seleccione un Proveedor.");
                if(this.voucher_type==0) this.errorMessage.push("El Campo Tipo de comprobante es obligatorio.  Por favor seleccione un Comprobante.");
                if(this.voucher_number==0) this.errorMessage.push("El Campo Número de comprobante es obligatorio.");
                if(this.tax==0) this.errorMessage.push("El CampoImpuesto es obligatorio.");
                if(this.arrayDetails<=0) this.errorMessage.push("Ingrese Detalle.");
            ...
            inactive(id){
                ...
                swal({
                    ...
                    if (result.value) {
                        axios.delete('/compra/' + id)
                        .then( function (response){
                            me.gerPurchases(1,'','name');
                            swal(
                                'Anulado!',
                                'Compra anulada con exito.',
                                'success'
                            )
                        }).catch( function (error){
                            console.log(error);
            ...
            showPurchase(id){
                ...
                axios.get(url).then(function (response) {
                    ...
                    let result = response.data;
                    me.provider = result.purchase.name;
                    me.voucher_type = result.purchase.voucher_type;
                    me.voucher_serie = result.purchase.voucher_serie;
                    me.voucher_number = result.purchase.voucher_number;
                    me.voucher_number = result.purchase.voucher_number;
                    me.tax = result.purchase.tax;
                    me.total = result.purchase.total;
                     console.log(result.purchase.voucher_type);
        ...
        mounted() {
            this.gerPurchases(1,this.search,this.criteria);
        }
